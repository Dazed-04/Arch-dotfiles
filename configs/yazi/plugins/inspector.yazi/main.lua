...
function M:peek(job)
	...
	local limit = math.max(1, job.area.h - image_height)
	...
	if output then
		...
		table.insert(
			lines,
			ui.Line({
				ui.Span("Media Info"):style(th.spot.title or ui.Style():bold()),
			})
		)
		...
		for str in output:gsub("\n+$", ""):gmatch("[^\n]*") do
			...
			if label then
				if not skip_labels[label] then
					...
					if #label < LABEL_COL_WIDTH then
						padded_label = label .. string.rep(" ", LABEL_COL_WIDTH - #label)
					else
						padded_label = label:sub(1, LABEL_COL_WIDTH - 1) .. "…"
					end

					line = ui.Line({
						ui.Span(padded_label):style(ui.Style():fg("reset"):bold()),
						ui.Span(": "):style(ui.Style():fg("darkgray"):bold()),
			...
			elseif str ~= "General" then
				line = ui.Line({
					ui.Span(str):style(th.spot.title or ui.Style():fg("green")),
				})
			end
			...
			if line then
				...
				if is_kv then
					local value_width = max_width - LABEL_COL_WIDTH - 2
					line_height =
						math.max(1, is_wrap and math.ceil(ui.width(ui.Span(value)) / math.max(1, value_width)) or 1)
				else
					line_height = math.max(1, is_wrap and math.ceil(ui.width(line) / max_width) or 1)
				end
