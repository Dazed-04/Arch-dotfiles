...
def main():
    ...
    lines = []
    lines.append(
        f"<span foreground='{SECTION_COLORS['Storage']['text']}'>{SSD_ICON} Storage Dashboard</span>"
    )
    lines.append(f"<span foreground='{COLORS['white']}'>{'─' * TOOLTIP_WIDTH}</span>")

    for entry in storage_entries:
        c_temp = (
            get_color(entry["temp"], "drive_temp")
            if entry["temp"]
            else COLORS["bright_black"]
        )
        ...
        lines.append(
            f"<span size='14000'><span foreground='{c_temp}'>{entry['icon']}</span> <span foreground='{COLORS['white']}'><b>{entry['name']}</b></span> - {size_str}</span>"
        )
        ...
        if entry["lifespan"] != "N/A":
            meta_info.append(f"Life: {entry['lifespan']}")
        if entry["tbw"] != "N/A":
            meta_info.append(f"TBW: {entry['tbw']}")
        meta_str = " | ".join(meta_info)

        lines.append(
            f" <span foreground='{c_temp}'>{temp_str}</span>  <span foreground='{health_c}'>♥ {entry['health']}</span>  <span size='small'>{meta_str}</span>"
        )
        ...
        lines.append(
            f"<span size='small'>R: <span foreground='{COLORS['blue']}'>{rs}</span>  W: <span foreground='{COLORS['green']}'>{ws}</span></span>"
        )
